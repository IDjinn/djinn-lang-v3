...
// values, variable assignment history) and native backtrace capture at throw
// sites. The descriptor layout must match djinn_error_info_t
// (runtime/djinn_runtime.h); trace symbolization is lazy and lives in the
// runtime (debug info + JIT-registered symbols).
//
...
llvm::Value* Generator::source_root_global()
{
    if (sourceRoot.empty())
        return llvm::ConstantPointerNull::get(llvm::cast<llvm::PointerType>(builder->getPtrTy()));
    return cached_global_string(sourceRoot, "err.srcroot");
}
...
// Captures the native stack at a throw site: the error unwinds normally
// afterwards, so this is the only chance to record where it came from for the
// uncaught-exception report. Symbolization happens lazily when the report is
// printed (dbghelp / llvm-symbolizer / JIT symbol table). Debug builds only.
void Generator::emit_error_trace_capture()
{
    if (runtimeDiagnostics == RuntimeDiagnostics::Minimal) return;

    auto* fnType = llvm::FunctionType::get(builder->getVoidTy(), false);
    auto* captureFn = get_or_declare_runtime_fn(module.get(), "__djinn_capture_backtrace", fnType);
    builder->CreateCall(captureFn);
}
...
// variable got its current value. Debug builds only — it runs on the hot
// path. Only the file id + line number are embedded; the runtime reads the
// line text from disk when (and if) a report is printed.
void Generator::emit_var_track(llvm::Value* slot, const std::string& name, const SourceLocation& loc)
{
    ...
    builder->CreateCall(trackFn, {
                            ...
                            cached_global_string(loc.fileId, "var.file"),
                            source_root_global(),
                            builder->getInt32(loc.line),
                        });
}
